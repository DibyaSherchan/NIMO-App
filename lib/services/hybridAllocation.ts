...
const LOCATION_TO_REGION: Record<string, 'central' | 'eastern' | 'western'> = {
  ...
  "Makwanpur": "central",
  "Dhulikhel": "central",
  "Kavrepalanchok": "central",
  "Banepa": "central",
  "Birgunj": "central",
  "Parsa": "central",
  "Gorkha": "central",
  "Sindhupalchok": "central",
  "Dhading": "central",
  "Dharan": "eastern",
  "Sunsari": "eastern",
  "Biratnagar": "eastern",
...
export function getDistributionStats(testCenters: TestCenter[]) {
  const stats = {
    ...
    byRegion: {} as Record<string, RegionStats>,
    balanceScore: 0
  };
  ...
  stats.overall.utilizationRate =
    (stats.overall.allocated / stats.overall.capacity) * 100;

  Object.keys(stats.byRegion).forEach(region => {
    const regionStats = stats.byRegion[region];
    regionStats.utilizationRate =
      (regionStats.allocated / regionStats.capacity) * 100;
  });
